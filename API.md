...
- **只管「帶 Bearer token 的認證嘗試」。** 累加與擋下都要求請求帶了 `Authorization: Bearer …`：
  - **不帶憑證的請求完全不受影響**——不會累加，被擋期間也不會被擋。包含未登入的瀏覽器、公開端點、以及 MCP 客戶端規範要求的「先發一個未帶憑證的請求換 `WWW-Authenticate`」握手。
  - **認證成功的請求不會累加**（不論多少次）。
- **認證失敗**指的是 401，**以及**「帶著 Bearer token 卻被導向登入頁」——後者是非 JSON 請求的失敗形狀（`Accept` 不是 JSON 時，未認證會 302 到登入頁而不是 401），一樣算。
...
- 送出的值可能被伺服器改寫後才落庫：**所有文本欄**都會先做 **Unicode NFC 正規化**（把相容表意文字折疊成統一表意文字，例如「慎」U+FA87 → U+614E；canonical equivalence，兩者本是同一個字，故**靜默、無 `notices`**），再依對照表做異體字替換（人名／別名欄用 strict＝較少規則集，其餘文本欄用全量規則 lenient）、拼音欄的 `v` 會歸一化為 `ü`。若被改寫的欄位剛好是主鍵欄（例如別名的 `c_alt_name_chn`），**回應 `result.pk` 才是真正落庫的主鍵**，請以回應為準，不要沿用送出的值。發生替換時回應會多一個頂層 `notices` 陣列。
...
- **create**（只支援 `direct`）可寫欄位：`c_personid`、`c_name_chn`、`c_name`、`c_name_proper`、`c_name_rm`、`c_surname_chn`、`c_mingzi_chn`、`c_surname`、`c_mingzi`、`c_surname_proper`、`c_mingzi_proper`、`c_surname_rm`、`c_mingzi_rm`、`c_female`、`c_index_year`、`c_index_year_type_code`、`c_index_year_source_id`、`c_index_addr_id`、`c_index_addr_type_code`、`c_dy`、`c_by_intercalary`、`c_birthyear`、`c_by_nh_code`、`c_by_nh_year`、`c_by_range`、`c_by_month`、`c_by_day`、`c_by_day_gz`、`c_dy_intercalary`、`c_deathyear`、`c_dy_nh_code`、`c_dy_nh_year`、`c_dy_range`、`c_dy_month`、`c_dy_day`、`c_dy_day_gz`、`c_death_age`、`c_death_age_range`、`c_fl_earliest_year`、`c_fl_ey_nh_code`、`c_fl_ey_nh_year`、`c_fl_ey_notes`、`c_fl_latest_year`、`c_fl_ly_nh_code`、`c_fl_ly_nh_year`、`c_fl_ly_notes`、`c_ethnicity_code`、`c_household_status_code`、`c_tribe`、`c_choronym_code`、`c_notes`
  - 白名單外的欄位會被**靜默丟棄**（不報錯，回應仍是 200）。
  - 這份清單與 **update** 可寫集合對稱：凡是 `mutate` 寫得進去的欄，`create` 也收得下，不需要「先 create 再 mutate 補寫」。（生卒年月日六欄 `c_birthyear`／`c_deathyear`／`c_by_month`／`c_dy_month`／`c_by_day`／`c_dy_day` 一度只有 update 收，現已補齊；兩端對稱由 `tests/Feature/MutationCreateUpdateParityTest.php` 機械把關。）反方向的差集只有姓名合併欄與 `c_personid`，理由見 update 段。
...
- 預設值必須是**字串或 `null`**，且長度 ≤ 255 字元，否則 422（訊息形如 `<欄名> 必須為字串或 null`／`<欄名> 長度不可超過 255 字元`）。放寬與收緊都是**逐表逐欄**登記的（`config/code_table_mutations.php`／`config/code_table_writes.php`，判定見 `App\Support\CodeTableFieldValidator`）：
  - **可送整數**：`TEXT_INSTANCE_DATA.c_pub_year`、`char_variant_map.c_strict_excluded`、`ADDR_CODES` 的 `c_firstyear`／`c_lastyear`／`c_admin_cat_code`／`CHGIS_PT_ID`、`ADDR_BELONGS_DATA.c_source`（訊息 `<欄名> 必須為字串、整數或 null`）。
  ...
  - **「只送一軸」在 create 與 update 的處理刻意不同**，這是唯一的不對稱，請照下表：
    - `create`：payload 就是要寫入的完整列，所以缺席的那一軸必然落庫成 `NULL`——`{"x_coord": 105.36}`（不送 `y_coord`）會被視為半截座標，**兩欄都寫 `NULL`**。
    - `update`：逐欄更新看不到資料庫，另一軸可能本來就有值，所以`{"x_coord": 105.36}` **原樣寫入**、不動 `y_coord`。若資料庫裡的 `y_coord` 是 `NULL`，這會留下一列半截座標 `105.36, NULL`——伺服器不會替你補齊，也不會擋。要避免就兩軸一起送。
...
限流：MCP 有自己的專屬額度（**預設 120 次／分鐘**，可由部署設定調整，但不會超過 600），獨立於其他 `/api/*` 端點共用的 600 次／分鐘（兩者互不排擠——打其他 API 不會消耗 MCP 的額度，反之亦然）。計數方式依請求而定：通過認證的 `POST` 按**帳號**計數；未帶憑證的 `GET` 按**來源 IP** 計數。

**注意**：未帶（或帶錯）憑證的 `POST` 會在認證階段就被擋下回 401，**不會**進到這個 120 次／分鐘的限流（框架的中間件順序是認證先於限流，這是全站 `auth` 路由的共同行為）。**帶錯**憑證的那種會受另一道「失敗認證」的額度限制（見 1.3）；完全不帶憑證的探測請求（MCP 握手用來換 `WWW-Authenticate` 的那一發）刻意不算在內。
