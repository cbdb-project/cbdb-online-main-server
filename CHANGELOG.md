...
### 複合主鍵與 URL 查詢模式
- 完成多個子資源改用「查詢參數模式」處理複合主鍵，降低 URL 編碼錯誤風險。
- 修正 `buildUrl()` 遺失 `null` 查詢參數導致編輯頁無法正確定位資料的問題。
- 修正社會關係、事件、財產、官名等子頁面在複合主鍵情境下的更新/刪除與導向異常。

### 任官、ALTNAME 與關聯資料一致性
- 任官（`POSTING_DATA` / `POSTED_TO_OFFICE_DATA` / `POSTED_TO_ADDR_DATA`）流程持續收斂為交易式處理，補強 `operation_id` 關聯一致性。
- 修正任官地址部分異動時 timestamp 被不必要覆寫、以及編輯時地址資料遺失的回歸問題。
- ALTNAME_DATA 完成 3-key 主流程切換與相容層落地，並修正 Operations/Restore 定位條件與過渡期遺留邏輯。

### Operations / Audit / 提案流程
- `/operations` 新增「修改人」「修改類型」篩選，改善多人操作追蹤效率。
- 改善多人關聯操作顯示，並修正親屬更新流程誤覆寫 `created_*` 欄位的問題。
- 補齊 ASSOC_DATA 鏡像操作的 audit 記錄，降低「操作記錄 vs 實際變更」落差。
- 修正提案核准時主鍵衝突假設，改為核准時重新分派並記錄實際主鍵；同步補強提案註解寫入與過濾。

### SQLite 匯出、遷移相容與維運
- 新增並迭代 SQLite 週期同步流程（含匯出、打包、上傳 HuggingFace、cron 環境相容）。
- 修正多筆歷史 migration 的 SQLite/MySQL 相容性與外鍵恢復問題（含 `SMALLINT` 調整與舊 ID 欄位移除遷移）。
- 新增 `db:export-to-sqlite` 的擴充選項與穩定性改進，降低資源消耗與失敗機率。

### 監控與平台能力
- 新增 Prometheus metrics 能力並修正記憶體與 cardinality 風險（例如 `__unknown__` 路由歸類）。
- 新增 MCP 唯讀介面（含 HTTP 介面提案與能力驗證）以支援受控資料查詢。
- 文件目錄重整：多份說明文件搬移至 `docs/`，並同步修正導覽與連結。
