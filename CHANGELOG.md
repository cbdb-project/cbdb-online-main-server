...
### 經緯度空白／零值一律落庫為 NULL，並清掉 ADDR_CODES 殘留的 316 列 0,0

- **回報**：自動化測試在 `ADDR_CODES` 發現 316 列 `x_coord = 0, y_coord = 0`，問「有沒有理由不把它們改成 NULL」。答案是沒有，而且比「顯示不出來」嚴重得多。
- **為什麼 `0,0` 是壞資料而不只是難看**：它不是東亞的任何地點，而是「沒有座標」被寫成了一個看起來合法的數字。讀取端早就這麼判（`CoordinateValidator` 判 `zero_axis` 為不可連結），但**判定端認定無效、寫入端照樣收**。而且零值會**主動製造錯誤答案**：舊版 v1 的鄰近地點查詢是 `ADDR_CODES.x_coord BETWEEN other.x_coord ± 0.03` 的自連接，所有 `0,0` 列因此互為「鄰居」——實測 316 列產生 99,856 對虛假鄰居。`NULL` 會自然從這種比較中掉出去，這正是這一欄要表達的語義。
- **生產資料已清理（2026-09-14，經 v2 API 逐列寫入、留下完整稽核）**：316 列裡 **12 列回復出真實座標**（3 列取自共用同一 `CHGIS_PT_ID` 的既有列——它們的小數位更多，是未被四捨五入的原值；9 列取自 CHGIS gazetteer），**304 列歸 NULL**。獨立讀回核驗：316 列全部重讀一次，仍為 `0,0` 的 **0 列**。第 13 個帶 `CHGIS_PT_ID` 的 700049 江西行省也歸了 NULL——**CHGIS 自己記的經度就是 `0.00000`**，那是 CHGIS 側的資料缺口，不是抓取失敗。
- **發現：現行入口沒有「預設值造成 0,0」的問題，但有另一個缺口。** 欄位是 `double DEFAULT NULL`；表單留白經全域 `ConvertEmptyStringsToNull` 變成 `null`；JSON 送 `""` 由 `CodeTableFieldValidator::normalize()` 轉 `null`。漏掉的是**顯式的零**：`0`、`"0"`、`"0.00000"` 在每一條路徑上都是合法浮點值，一路落庫成 `0`。而那 316 列幾乎確定是原始匯入帶進來的（散佈整個 id 區間、涵蓋明代衛所等一整批同類地名，看起來是一次匯入的產物而非逐筆編輯）。**不要用稽核欄去推論這件事**：`c_created_by`／`c_modified_by` 是 `2026_09_10` 那支 migration 才加的 nullable 欄且不回填，所以每一列既有資料必然是 NULL，那些 NULL 只證明「加欄之後沒被編輯過」。
- **新增寫入端守衛**（`App\Support\CoordinatePairNormalizer`，規則見 AGENTS.md §1.4）：空白或精確為零的經緯度落庫成 `NULL`，**以「對」為單位**——任一軸符合就整對清空，包含呼叫端沒送的那一軸（只有一軸的座標對每個消費端都不可用）。這讓 `result.updated_fields` 可能出現呼叫端沒送的欄位，是對外契約變更，已同步 `API.md` 與 `docs/openapi/openapi.yaml`。
- **掛鉤覆蓋十二條寫入路徑**：v2 的 update 與 create（proposal 與 direct 共用同一個 handler，所以那兩條共用一個 trait 掛鉤）、提案核准的 create 與 update 兩條重放、`/codes` 與 `/app/codes` 的五個表單方法（`performUpdate`／`performStore`／`performProposalStore`／`performProposalUpdate`／`proposalUpdateExisting`）、operations 的 `restoreUpdate`／`restoreDelete`，外加後備清掃 service 自己那一條。只掛 API 那幾條不夠——表單其實才是這張表的主要人工寫入端，而核准重放會把**上線前送出的舊 payload** 裡的 `0` 寫回來。
- **「零」用精確比較，刻意不讀 `config('chgis_map.epsilon')`**：那是 `CHGIS_MAP_EPSILON` 可調的讀取端容差，讓它驅動破壞性寫入等於把地圖調參變成刪資料的開關（`CHGIS_MAP_EPSILON=1` 會靜默刪掉每一個 `|lon| < 1` 的座標）。兩邊的關係是**包含**，由測試斷言而非共用旋鈕保證。這是審查抓到的，第一版真的讀了那個 config。
- **非數值的值不歸零、交給驗證層 422**（`"east"`、`"0e0"`、`"+40.5"`）——靜默清成 `NULL` 會把該報錯的請求變成「存成沒有座標」。**但那個前提只在 v2 API 上成立**：`CodesController` 與核准重放都沒有欄位驗證層，`"0e0"` 會被非 strict sql_mode 的 MariaDB **靜默轉成 0**（實測）。那兩條路因此自己呼叫 `invalidColumns()`——表單回欄位錯誤、核准／還原中止。刻意**不**在表單路徑套用整份 `CodeTableFieldValidator`：它的型別清單是照 v2 `allowed_fields` 寫的，而表單會把整列送回來，update 端那份 config 只有兩張表宣告 `long_text_fields`，套上去會讓使用者在一列備註超過 255 字的資料上改任何無關欄位都硬 422（編輯表單會把既有 `c_notes` 原樣 post 回來）。
- **`restore` 也歸零，刻意不同於 §1.3 對異體字的 restore 豁免**：那條豁免存在是因為「歷史字形本身就是事實」，改寫還原內容是資訊的損失；`0,0` 不承載任何資訊，還原它是**重新武裝一個 bug**。而 restore 本來就不是位元級忠實重放——同一個方法下面幾行就會把 `c_modified_*` 蓋成還原人與此刻（§1.2）。
- **後備機制**：`php artisan cbdb:normalize-addr-coords`（幂等，**每次上游匯入後都該跑**）與共用同一份邏輯的一次性 data migration，讓每個部署與每個 migrate-from-scratch 的環境都自動到達「沒有零座標」這個地板。那支清理刻意不寫 `operations`／`audit_log`、不蓋 `c_modified_*`——它是資料清理而非 316 次編輯行為，逐列寫稽核會用一批「0 → NULL」的無資訊列淹掉 operations 頁，而把最後修改者改成執行清理的人會抹掉「這列上次真的被誰改過」這個更有價值的事實。`down()` 是 no-op 且刻意如此：把 `NULL` 還原成 `0` 會重新造出這套機制要防的那一列，而且沒有記錄哪些列被改過，無法只還原那些列。
- **順帶修掉一個既有的洞**：`CodeTableFieldValidator` 對 float 欄沒有 finiteness 檢查，`json_decode('{"x_coord":1e999}')` 就是 `INF`，原本兩層都穿得過去、一路寫進 double 欄。現在回 422「必須為有限數值」。
- **順帶改掉一處承重的隱性契約**：`CharVariantMapService::withNotices()` 原本是 assign 而非 merge，於是「先掛座標、再掛異體字」的組合順序會**靜默吃掉**座標通知——而順序正確性沒有任何測試守得住（實測把順序反過來，整個套件照樣全綠）。兩邊都改成 merge。
- **審查過程值得記一筆**：四輪 review agent ＋ 兩輪 codex，抓到的東西包括上面那個 env 可調的破壞性開關、一個**我自己引入的通知洩漏回歸**（把兩處 `tree_cycle` 422 改成掛通知時，它們排在累積器重置之前，於是批次的第二筆會揹著第一筆的「你送的『淸』已正規化」——而當時的註解宣告那條路徑不可達，那份只對一半成立的自信正是掩蓋 bug 的東西）、一支空轉的「409 仍帶通知」測試（實測回 200 且沒斷言狀態碼）、以及五處描述了不存在機制的註解。
- **測試**（數字為實跑案例數，含 data provider 展開）：`CoordinatePairNormalizerTest`（58）、`CoordinatePairRegistryGuardTest`（4，機械把關登記與白名單／schema 漂移，四種表定義寫法都掃）、`CodeTableFieldValidatorFiniteNumberTest`（9）、`ApiV2AddrCodesZeroCoordinateTest`（22）、`ApiV2BatchNoticeIsolationTest`（3）、`CodesUiZeroCoordinateTest`（21）、`NormalizeAddrCoordsCommandTest`（9）。每一組掛鉤都以 mutation 驗證過移除時會變紅。

### v2 API 補上官職類型層級樹（OFFICE_TYPE_TREE）的寫入，順帶讓代碼表寫入支援文本主鍵

- **回報**：「完全沒有寫入口徑」。這張表原本只能走 `/codes` UI 或眾包端，v2 API 完全碰不到。
- **為什麼不是「再登錄一張表」就好**：`config/code_table_writes.php` 原本明文限制「只能登錄數值主鍵的表」——handler 一律把主鍵 `(int)` 轉型。而 `c_office_type_node_id` 是**零填補的階層路徑字串**（`06`、`0601`、`060102`，全庫 2739 筆都是 ASCII 數字串），轉型會把 `'06'` 變成 `6`，把節點建在一個看起來也很合理的錯鍵上——回應說建好了、資料在別的地方。所以這次是把 create／update／delete 三條路徑都擴到**依實際 schema 判斷主鍵是數值還是文本**：文本主鍵不轉型、驗長度（依欄位實際 varchar 長度）、回應也原樣回字串。（delete handler 目前開頭無條件回 403，那段是為了日後恢復刪除時可用而一起改的、目前不可達。）
- **D7 前提由程式重新驗證、不是靠註解**：config 那段警告要求文本主鍵必須先處理「兩形並存查重」。這張表之所以能放行，是因為 `c_office_type_node_id` 與 `c_parent_id` 本來就在 `VariantReplaceScope` 的排除清單內（它們是跨表 join 的代碼鍵），替換永遠不會動到主鍵。handler 現在會 fail-closed 地檢查這件事：文本主鍵若落在替換範圍內直接回 500 設定錯誤。文本主鍵同時禁止 `auto_assign_id`（`'060102' + 1` 不是一個階層路徑）。
- **自參照樹的環路守衛**：`c_parent_id` 有自參照外鍵，所以「父節點必須存在」由資料庫保證（違反 → 422 `foreign_key_violation`）。但**自己當自己的父節點**滿足外鍵，`A→B` 與 `B→A` 也各自滿足外鍵。成環的後果是任何「沿 `c_parent_id` 往上找根」的走訪都會無窮迴圈——**本 repo 目前沒有程式讀 `c_parent_id`**（站內的階層查詢是對 **id 字串**做 `LIKE '<id>%'` 前綴比對），所以這是保護資料本身與外部消費者的守衛，不是修某個現有頁面的 bug；這張表是對外釋出資料的一部分，而成環的樹沒有任何消費者能安全走訪。新增 `changes: ["tree_cycle"]` 422：擋自我引用、擋「搬到自己的子孫之下」、也擋「掛到一個本來就在環上的分支」。走訪有 200 層上限，撞到上限一律拒絕（fail-closed，否則守衛自己會無窮迴圈）。比較前先 trim + casefold：資料庫 collation 是 `utf8mb4_general_ci` 且 PAD SPACE，`'0601 ' = '0601'` 在 DB 端成立，用 PHP `===` 比原值會讓帶空白的自我引用溜過去。
- **實測抓到的設計錯誤**：這張表**以「自己是自己的上層」表示根節點**（prod 的 `'0'` 的 `c_parent_id` 就是 `'0'`）。守衛第一版把它當成環，於是每一個祖先鏈通到根的節點——也就是全部——在新增或改上層時都被誤擋。用比照 prod 的種子資料寫測試才浮出來；現在走訪碰到 `parent(x) === x` 判定為「走到根」，並且上層沒有實際改變時完全不驗（根節點整列原樣存回不該被擋）。兩種情形都有專門的迴歸測試。
- **守衛必須蓋到六條寫入路徑**：`/api/v2/create`、`/api/v2/mutate` direct、**提案核准**、**operations 還原**、**`/codes` UI 的新增與修改**、**眾包核准**（由仍在服役的 v1 token API 餵入）。只掛 API 那兩條是不夠的，而且不夠的方式很具體——提案會躺好幾天，提交時合法的「把 06 掛到 0699 之下」，核准時可能已經因為別人把 0699 搬到 06 之下而變成環；還原舊快照同理；而 `/codes` UI 其實才是這張表的**主要**寫入者，根本不經過 API 的守衛。判定因此抽成 `App\Support\SelfReferencingTreeGuard`（後四條路徑不屬於 handler 體系，放在 trait 裡它們拿不到）。這條規則沒有機械化把關，新增寫入路徑時只有 code review 擋得住，類註有寫。
- **核准路徑必須拿「現況」而不是「提案當時的快照」比對**（第二輪 review 抓到的 blocking bug）：代碼表提案存的是整列合併結果，所以 `c_parent_id` 一定在 payload 裡、核准時一定會被寫回——即使那是一個只改說明的提案。於是「上層沒變就不驗」的短路命中的正好是最危險的情境：提案只改說明（帶 `c_parent_id='06'`）→ 期間有人把 `0601` 搬到根、又把 `06` 搬到 `0601` 之下（兩步各自合法）→ 核准時 payload 的 `'06'` 等於快照的 `'06'`、短路跳過 → 寫下 `0601→06`，而 `06→0601` 已在庫裡，環成立。改成拿現況比對後這一步會走進走訪並擋下，而根節點（現況上層等於自己）仍然短路、不會誤擋。有專門的迴歸測試把兩個方向都釘住。
- **主鍵型別在 update 端也要處理**（第一版漏了，review 抓到）：MariaDB 比較 varchar 與數字時會把欄位轉成數字，所以 `where('c_office_type_node_id', 601)` 會**命中 `'0601'` 那一列**（已對真實庫實測）。沒轉字串的話 UPDATE 改到對的列，但 `resource_id`、`audit_log.row_pk` 與回應的 `result.pk` 全部記成 `601`——一個沒有任何列擁有的鍵：operations 頁解不出現況，呼叫端照回應的鍵做後續操作一律 404。SQLite 不做這種轉型，所以這是**測試環境永遠看不到**的一類。
- **update**：`c_office_type_desc`／`c_office_type_desc_chn`／`c_parent_id` 三欄。主鍵不在白名單內——改主鍵等於把節點搬到樹的另一個位置，須另行新增／刪除。中文說明欄照型別驅動做落地替換（它只用於顯示與 `LIKE` 搜尋，不是 label→code 查表鍵）；主鍵與 `c_parent_id` 不替換（單邊歸一會打斷 `OFFICE_CODE_TYPE_REL` 的關聯）。
- **連帶**：`OFFICE_TYPE_TREE` 補進 `CompositePrimaryKey::SCHEMAS`（原本只在 `OperationsController::resourceKeyColumns()` 有登錄）。刪除維持停用（403）——本表被 `OFFICE_CODE_TYPE_REL.c_office_tree_id` 與自身的 `c_parent_id` 引用。
- **並發（TOCTOU）**：守衛本質上是「檢查後才寫」——兩個同時進行的 `A.parent=B` 與 `B.parent=A` 各自都會看到對方還掛在根上而通過，兩邊都提交、環成立。v2 的 create 與 update 因此在**寫入交易之內**再驗一次，並鎖住走訪路徑上的每一列（`SELECT … FOR UPDATE`）；被擋下時以專屬的 `TreeCycleException` 回滾並轉成 422 `tree_cycle`（不與資料庫例外混在一起）。鎖定走訪的代價是兩個方向相反的重掛會以不同順序取鎖、資料庫可能挑一方回滾——那是**正確**的（沒寫出環），但不該以 500 呈現，所以 deadlock／lock wait timeout 另外分類成 409 `lock_contention`（可直接重試）。其餘四條路徑只有無鎖的單次檢查——它們都是單一管理者的互動操作，交易化會動到與本功能無關的錯誤處理與 redirect 流程，屬已知殘留風險、非遺漏（類註與 API.md 都寫明）。
- **鍵值比較交給資料庫**：這張表的 collation 是 `utf8mb4_general_ci`（不分大小寫、PAD SPACE、部分重音形視為相等）。用 PHP 比原值或只做 trim + casefold，都可能把「資料庫眼中相同的兩個鍵」判成不同而放行，然後自參照外鍵把它解析到同一列——那一列就成了自己的父節點。改成先用 PHP 短路（絕大多數呼叫在此就有答案），PHP 判成不同時**把兩個值直接在 SQL 裡用該欄的 collation 比一次**（`SELECT (? COLLATE <coll>) = (? COLLATE <coll>)`；collation 取自 `Schema::getColumns()` 並白名單化字元集，因為 `COLLATE` 不吃 bound parameter）。中間曾用「拿欄位問兩次、看有沒有列同時滿足」，但那個寫法**漏掉 create**——新節點的列還不存在，於是「找不到列」被當成「兩個值不同」而放行，然後自參照外鍵在插入後把父鍵解析到剛插進去的那一列，環就成了（第三輪 codex 抓到；已對真實庫實測 `'À' = 'a'` 在 utf8mb4_general_ci 之下為真）。拿不到 collation 時（SQLite）回退成 PHP 精確字串比對——SQLite 預設比對是 BINARY，與 `===` 同義，所以那條回退是精確的、不是猜的。短路**只敢往「資料庫也一定認為相等」的方向**：PAD SPACE 只忽略尾端空格，所以只能 rtrim 不能 trim——用 trim 的話 `' 06'` 與 `'06'` 會被誤判成同一個鍵，把一個合法的寫入擋成自我引用（第二輪 codex 抓到）。
- **其他 review 修正**：型別讀不到整張表時 create **與 update** 都直接 500 `pk: schema_unavailable`（`isTextColumn()` 在讀不到時回 false，於是文本主鍵會被 `(int)` 轉型建到錯鍵上，而 D7 與 auto_assign 兩個守衛也都掛在它之下、會一起無聲失效）；create 的環路守衛移到落地替換**之前**以與 update 端同序（今天 `c_parent_id` 在替換排除清單內所以等價，但若那個排除被移除，「驗替換前的值、落庫替換後的值」正是 AGENTS §1.3 點名的陷阱）；`config/code_table_writes.php` 的「只能登錄數值主鍵」檔頭已同步改寫。
- **測試**：`ApiV2MutateOfficeTypeTreeTest`（27 案例，含一支**端到端的核准**測試——上面那個 blocking bug 在「呼叫端傳了哪個參數」，純呼叫守衛的單元測試擋不住，必須真的走一次提案→他人改樹→核准，表用原生 DDL 建才看得到 `varchar` 與自參照外鍵：型別可見性前提、零填補主鍵原樣落庫且不產生 `'6010304'`、重複鍵 409、超長鍵 422、缺鍵 422、自我引用與子孫環路各 422、合法搬移成功、根節點慣例兩案、說明欄更新、拒改主鍵、中文欄替換並回 `notices`、update 送數字主鍵時記錄的鍵必須是字串（`result.pk`／`operations.resource_id`／`audit_log.row_pk` 逐一斷言）、既有環上的分支拒絕掛載、帶空白／大小寫不同的自我引用仍被擋、只登錄在單一份 config 的表也查得到 `tree_parent_column`、整列覆寫守衛拿現況比對而非快照、上層真的沒變時仍然短路、只在 collation 之下相等的鍵在該列尚不存在時也要擋（用 SQLite 的 `COLLATE NOCASE` 重現同一結構，走的是同一段程式碼）、前導空白不可誤判成同一個鍵而尾端空白必須）。
...
- **新增能力**：`ADDR_CODES` 支援 `create`（主鍵可自動配發或指定）與 11 欄整列 `update`（`c_name`／`c_name_chn`／`c_alt_names`／`c_firstyear`／`c_lastyear`／`c_admin_type`／`c_admin_cat_code`／`x_coord`／`y_coord`／`CHGIS_PT_ID`／`c_notes`）；`ADDR_BELONGS_DATA` 支援四欄複合主鍵的 `create` 與 `update`（`c_source`／`c_pages`／`c_notes`）。兩者都走既有授權 + `operations` + `audit_log`，可回滾、可放進 `batch_mutate`、可用 token。刪除維持全面停用（403），與其他代碼表一致。
- **create／update 兩端對稱**：`ADDR_CODES` 的 update 白名單同步擴到與 create 相同的 11 欄——只開 `c_name` 會變成「新增時填得進去、之後改不了」，與 AGENTS §4 的必填欄位規則同一類問題。新增 `CodeTableWriteConfigDriftTest` 機械把關：同時登錄在兩份 config 的表，白名單與四組型別登記必須一致（刻意的差異要登記理由），且欄位與型別登記必須與 migration schema 對得上。這條線 `MutationCreateUpdateParityTest` 涵蓋不到（它掃的是 handler 的欄位常數，看不到 config 驅動的多表分派），該測試裡那條「create/update 共用同一份 config 白名單」的過時理由一併改正。
- **create 端補上型別校驗**：抽出 `App\Support\CodeTableFieldValidator`，讓 create 與 update 共用同一套判定（整數／浮點／longtext 不套 255 上限／NOT NULL 不可清空）。新增端原本**完全沒有校驗**，型別不合是資料庫層 500；現在是 422。相容既有呼叫端：**create 端**文字欄收到 JSON 數字時轉成字串再寫入（等同資料庫原本的隱式轉型），不會把既有的外部 token 客戶端變成 422；**update 端維持嚴格**（`ApiV2MutateCodeTablesTest` 有一支測試專門釘「integer_fields 的放寬不得波及未登記欄位」，不為了對稱把它拆掉）——這個不對稱是刻意的，已寫進 API.md 與 `CodeTableFieldValidator::normalize()` 的類註。整數欄則兩端都接受「小數部分為 0 的浮點數」（`1200.0` → `1200`，JSON 序列化器常這樣送整數），真帶小數時 422、不靜默截斷。外鍵指向不存在的列（上級地名尚未建立、`c_source` 指向不存在的文獻）由 1452／SQLite 訊息判定轉 422 `foreign_key_violation`，不再冒成 500——**create 與 update 兩端都做**（update 端這次新開放的 `c_admin_cat_code`／`c_source` 就是外鍵欄，只做 create 等於留一半）。NOT NULL 違反同理轉 422，並把各表實際的 NOT NULL 欄補齊登記；三種違反在 SQLite 共用 errno 19，判定順序與訊息互斥收在 `Concerns\HandlesCodeTableWrites`。數值欄送字串時必須真的是數字，否則 422——本專案 sql_mode 非 strict，`c_firstyear: "not-a-year"` 原本會被 MariaDB **靜默存成 0**，而 0 是合法年份、事後看不出是壞資料；可為 null 的數值欄送空字串則正規化成 null（原本同樣會變成 0，把「清空」曲解成「填 0」）。整數欄另依**實際 schema 型別**做值域檢查（smallint 收到 40000 會被非 strict 的 MariaDB 靜默截斷成 32767——截斷是 warning 不是 exception，例外兜底抓不到）；值域從 schema 推導、不寫進 config，避免又多一份手抄清單；**主鍵也驗值域**——主鍵在白名單化時就被抽出去、不經欄位校驗，漏掉的話那一列會被建在呼叫端沒有指定的鍵上（回應說 40000、實際 32767，之後照回應的鍵去改／刪一律 404）。超出 PHP 整數精度的字串另有溢位檢查（`(int)` 會飽和成最大值而非報錯，會讓值域檢查誤判為「剛好在範圍內」）。strict 部署才會拋的 1264／1366／1406 也一併兜底轉 422。主鍵值非整數回 422 `numeric`，不再 `(int) 'abc'` 靜默變成 0（0 在 CBDB 的年份欄是合法值，靜默轉型會生出事後看不出異常的錯誤列）。
- **稽核欄（AGENTS §1.2）**：新增 migration 為 `ADDR_CODES` 補上 4 個稽核欄（姊妹表 `ADDR_BELONGS_DATA` 早在 2025-11 就有）；`CodeTableCreateHandler` 改為按實際欄位蓋章，往後登錄缺欄的表不會再炸。同時修掉一個既有缺口：代碼表 `update` 路徑從不蓋 `c_modified_by`／`c_modified_date`，但同一列改走 `/codes` 表單就會蓋——`c_modified_by` 會依「最後是哪個介面改的」而時對時錯，比全部留空更難查。現在兩條路徑一致，署名一律經 `AuditActor`。
- **落地替換（AGENTS §1.3）**：兩張表都是資料表、不在任何排除清單，新開放的中文欄（`c_name_chn`／`c_alt_names`／`c_notes`）自動經型別驅動的替換；主鍵全是數值欄，「替換必須早於 PK 計算」在這條路徑上自然成立。順帶補上 409 與新的外鍵 422 也要帶 `notices`（原本只有成功路徑帶），否則使用者被擋下時不知道自己輸入的字已被正規化。
- **tier 語義釐清**：`tier1_fields ∪ tier2_fields` 不再等於 `allowed_fields`，改為子集。兩個 tier 只涵蓋拼音／羅馬字欄——`ADDR_CODES` 只有 `c_name`；把中文名、年份、座標、註記塞進 tier2 只為湊相等，會讓 `/codes` 編輯器對它們也彈 ü 轉換視窗，也會被 `cbdb:migrate-code-pinyin-v --tier=tier2` 掃到。
- **連帶影響**：`ADDR_BELONGS_DATA` 登進 `CompositePrimaryKey::SCHEMAS` 與 `OperationsController::resourceKeyColumns()` 後，既有由 `/codes` 產生的該表 operations 列**首次具備「還原」能力**（原本一律 `restore_no_pk`）。`/codes/ADDR_CODES` 的列表與編輯頁會多出 4 個唯讀稽核欄（既有列為空），與姊妹表一致。
- **已知限制**：(1) 代碼表**可以提案修改、但不能提案新增**（create 只支援 `direct`），眾包帳號只能改不能增；(2) 代碼表不支援 `/api/v2/read`，新增後的主鍵看回應的 `result.pk`／`result.row`；(3) `ADDRESSES` 是由這兩張表派生的反正規化快取，只由 `php artisan cbdb:regenerate-addresses-table` 重建——新增的地名在重建前不會出現在官職地點自動填表、朝代同名地消歧與部分自然語言查詢（`/codes` UI 本來就有同樣的落差，但機器化批次新增會放大這個時間窗）。以上三點都已寫進 `API.md`。
