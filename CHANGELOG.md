...
- **發現：現行入口沒有「預設值造成 0,0」的問題，但有另一個缺口。** 欄位是 `double DEFAULT NULL`；表單留白經全域 `ConvertEmptyStringsToNull` 變成 `null`；JSON 送 `""` 由 `CodeTableFieldValidator::normalize()` 轉 `null`。漏掉的是**顯式的零**：`0`、`"0"`、`"0.00000"` 在每一條路徑上都是合法浮點值，一路落庫成 `0`。而那 316 列的 `c_created_by`／`c_modified_by` 全部是 `NULL`——**從來沒被應用寫過**，是當年原始匯入帶進來的。
...
- **掛鉤覆蓋九條寫入路徑**：v2 的 create／update／proposal、提案核准重放、`/codes` 與 `/app/codes` 的五個表單方法、operations 的 `restoreUpdate`／`restoreDelete`。只掛 API 那幾條不夠——表單其實才是這張表的主要人工寫入端，而核准重放會把**上線前送出的舊 payload** 裡的 `0` 寫回來。
...
- **測試**：`CoordinatePairNormalizerTest`（53）、`CoordinatePairRegistryGuardTest`（4，機械把關登記與白名單／schema 漂移，四種表定義寫法都掃）、`CodeTableFieldValidatorFiniteNumberTest`（9）、`ApiV2AddrCodesZeroCoordinateTest`（22）、`ApiV2BatchNoticeIsolationTest`（3）、`CodesUiZeroCoordinateTest`（13）、`NormalizeAddrCoordsCommandTest`（8）。每一組掛鉤都以 mutation 驗證過移除時會變紅。
