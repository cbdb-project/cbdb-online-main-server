...
```bash
php artisan db:export-sqlite database/test.sqlite
```
...
DB_DATABASE=/var/www/html/database/test.sqlite
```

**重要**：路径必须是容器内的路径 `/var/www/html/database/test.sqlite`，不是本地路径。
...
**方式二：在容器内查看**
```bash
docker compose exec php sqlite3 /var/www/html/database/test.sqlite
```

### 备份数据库
```bash
cp database/test.sqlite database/test.sqlite.backup
```
...
检查 `.env` 文件：
- `DB_CONNECTION=sqlite`
- `DB_DATABASE=/var/www/html/database/test.sqlite` （容器内路径，不是本地路径）
- 确保 `database/test.sqlite` 文件存在且有读写权限
