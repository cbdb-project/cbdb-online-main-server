...
- **AdminLTE 3** (Bootstrap 4) + Blade 仍實體保留，但**已全面封路**：舊 URL 一律 302 導向 `/app` 對應頁、legacy 寫入端回 410。🔴 **這批頁面的回退鍵不再是 migration flag**——請設 `LEGACY_PAGE_RETIREMENT=false` 並 `php artisan config:clear && php artisan config:cache`（不需重新部署）。**例外：認證頁與首頁（`auth.*`／`welcome`）未封路**，它們的 flag 分支在 controller 內部，翻 `MIGRATION_FLAG_AUTH_*`／`MIGRATION_FLAG_WELCOME` 仍會渲染 Blade。人物編輯全套（`basicinformation.*`）與**唯讀頁**（`operations`／`dashboard`／`view`／`merge-preview`／`crowdsourcing`／`nl-query-logs`／`admin.audit-logs`／`admin.ai-fill-logs`）已**實體刪除**、無法回退（舊 URI 只剩 302 導向）。仍可用 kill switch 叫回的只剩表單／寫入頁。逐條清單見 [docs/BLADE_RETIREMENT_STAGE3_ROUTE_MANIFEST.md](docs/BLADE_RETIREMENT_STAGE3_ROUTE_MANIFEST.md)。AdminLTE 實體下架未執行。新功能一律只做在 React/Inertia 路徑。
...
- 舊版 Blade 路由已全面封路（顯示頁 302、寫入端 410）。**唯讀頁的視圖與 controller 已於環節 4a 實體刪除**；表單／寫入頁的碼仍保留；回退用 `LEGACY_PAGE_RETIREMENT=false`（**不是** migration flag）。認證頁與首頁未封路，仍由 `MIGRATION_FLAG_AUTH_*`／`MIGRATION_FLAG_WELCOME` 決定渲染。人物編輯全套已實體刪除。Query Playground 與外部資料庫引用瀏覽器本就硬導向 React、無 flag。皆不再新增功能；新功能一律做在 React/Inertia。
- 前端入口：
