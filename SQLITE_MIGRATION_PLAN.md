...
**重要提示：**
- 🔒 **数据完整性保证**：
  - 总是使用排序确保数据完整导出，不会跳过或重复行
  - 自动检测表结构，只在单列主键时使用 `chunkById()`
  - **复合主键表使用所有主键列排序**：如 KIN_DATA (c_kin_code, c_kin_id, c_personid) 会按三列排序，确保稳定的排序结果，避免 UNIQUE constraint 错误
  - 无主键表自动使用第一列排序
- ⚡ **智能分块策略**：单列主键表使用高效的 `chunkById()`，其他表使用安全的多列 `orderBy() + chunk()`
- 🎯 **自动磁盘检查**：导出前会检查输出目录和 `/tmp` 的可用空间
- 💾 **内存优化**：分批读取数据，定期释放内存，适合大表导出
