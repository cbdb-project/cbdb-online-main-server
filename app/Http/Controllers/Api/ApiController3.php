...
class ApiController3 extends Controller {
    ...
    protected function query_entry_postings(Request $request) {
        ...
        $entry = $peoplePlace = $data = $useXyArr = [];
        ...
        if ($useDate) {
            if ($dateType == 'entry') {
                if (empty($dateStartTime) || empty($dateEndTime)) {
                    return 'Plaese check dateStartTime and dateEndTime have value.';
                }
                ...
                if (empty($dynStart) || empty($dynEnd)) {
                    return 'Plaese check dynStart and dynEnd have value.';
                }
