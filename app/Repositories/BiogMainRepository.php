...
class BiogMainRepository {
     ...
     *  - 但精確比對連帶擋掉了所有帶括號尾巴的名字，搜 "Guo Shi" 找不到
     *    "Guo Shi (Wife of Zhao Zhen )"。本方法只修這一點。
     *  - **「數字消歧後綴」已不再是它存在的理由**；現在的理由是「說明性括號」
     *    （`(Wife of …)`、`(zi)` 之類，dev 上約 54,500 列）以及一般多詞姓名的詞前綴搜尋
     *    （搜 "Jia" 要能命中 "Jia Gongyan"，那是 `$form.' %'` 那一支）。
     *    2026-09-22 的 migration
     *    `2026_09_22_000000_strip_numeric_disambiguation_suffix_from_biog_main_names`
     *    已把「數字消歧後綴」那 5,185 列從 c_name 裡刪掉了，所以**不要再拿 "Jia Gongyan (2)"
     *    當這裡的理由**——那種資料已經不存在（本系統是資料端最上游，也不會回灌）。受影響的列
     *    現在改由第一條 `like $form`（沒有附加萬用字元的那一支）命中——**對乾淨名字的查詢而言
     *    命中集合不變**，也不會重複命中（`namesByQuery` 有 `groupBy(c_personid)`，而分面與列表
     *    共用本方法）。但用**帶後綴的字串**查詢會從 1 命中變 0 命中，見 CHANGELOG。
     *  - **仍然不能退回 #154 的整值精確比對**：說明性括號那批還在，退回去搜 "Guo Shi" 又會找不到
     *    人，那是比消歧後綴大十倍的回歸。要等括號內容整批搬到獨立語義欄位之後才能退。
     *  - 兩種邊界都要留：Access 主檔的寫法是括號前**沒有**空格（`Guo Shi(Wife of …)`），
     *    prod 曾整批補過空格成有空格形，兩種接法在資料裡並存，而 BracketNormalizer 只管新寫入。
     *  - 「q 後接 (」是**邊界**而不是「(n) 後綴」的特例：括號後面是什麼不管，"(Wife of …)"、
