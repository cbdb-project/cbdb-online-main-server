...
class PostingMutationHandler extends AbstractPersonSubresourceMutationHandler {
    ...
    /**
     * 「未詳」人物（personid 0）不得**修改**任官記錄。
     *
     * `PostingCreateHandler:106` 早就擋了 create，update 這一側一直沒有對應守衛——
     * legacy `BasicInformationOfficesController` 兩側都沒擋（grep 該檔已刪版本的「未詳」
     * 得 0 筆），所以這不是遷移漏搬，而是 v2 自己補到一半。對齊
     ...
     * 同資源的 create 路徑逐字相同（`['person_id' => ['invalid']]`）。
     *
     * 歷史上已存在的 personid 0 髒列因此變成「只能刪、不能改」——與 kin／assoc 的既有
     * 取捨一致（c_personid 是主鍵成員，本來就改不動）。
     */
    private function blockUnknownPostingOwner(int $personId): ?JsonResponse {
        if (!UnknownPerson::isUnknown($personId)) {
            return null;
        }

        return $this->errorResponse('「未詳」人物不能修改任官記錄。', 422, ['person_id' => ['invalid']]);
    }
    ...
    protected function handleAfterVariantReset(string $resource, string $mode, string $operation, int $personId, array $targetPk, array $changes, array $meta = []): JsonResponse {
        ...
        if ($blocked = $this->blockUnknownPostingOwner($personId)) {
            return $blocked;
        }
