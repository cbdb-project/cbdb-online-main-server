...
class DropViewAddressView extends Migration {
    public function up() {
        // 刪除 View_Address 視圖
        DB::statement("DROP VIEW IF EXISTS View_Address");

        // 刪除之前建立的索引（如果仍需保留這些索引可註解掉以下部分）
        Schema::table('ADDR_BELONGS_DATA', function (Blueprint $table) {
            // 檢查索引是否存在再刪除，避免錯誤
            if (Schema::hasColumn('ADDR_BELONGS_DATA', 'c_belongs_to')) {
                $table->dropIndex('idx_belongs_to');
            }
            if (Schema::hasColumn('ADDR_BELONGS_DATA', 'c_addr_id')) {
