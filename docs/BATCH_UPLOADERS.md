...
- 使用 `VariantCharNormalizer` 標準化異體字後，透過 `App\Services\PinyinDictionary` 生成 `c_title`（拼音）；異體字（如「菴」→「庵」）會在拼音轉換時自動標準化，但原始書名保持不變；非漢字會保留原字詞。
...
- 先檢查名稱是否已存在：
  ...
  - 若不存在，分配新 `c_inst_name_code`、使用 `VariantCharNormalizer` 標準化異體字後以 `App\Services\PinyinDictionary` 生成拼音並建立名稱紀錄。
...
- 使用 `VariantCharNormalizer` 標準化異體字後，以 `App\Services\PinyinDictionary` 轉換中文職名為 `c_office_pinyin`。
