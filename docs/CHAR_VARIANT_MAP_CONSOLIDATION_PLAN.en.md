...
**Collation follows the database default; no binary collation is specified**: `c_variant_char`/`c_reference_char` do not explicitly set a binary collation (e.g. `utf8mb4_bin`); they follow the project's default (`utf8mb4_general_ci`/`utf8mb4_unicode_ci`). This matches the `pinyin` table's `c_chn` column (also a single-CJK-character field looked up via an index — in fact a composite unique key on `(c_chn, c_lastname)` plus a separate plain index on `c_chn`; see `2026_07_10_000000_restructure_pinyin_table.php:79-80`) — `ci` collation's equivalence folding mainly affects case/diacritic comparisons for Latin scripts, and poses no risk of misjudging the unique key or query results for the distinct Han characters covered by this table's current 7 seed rows (e.g. "峯" vs "峰"). **Known limitation**: if future entries involve variation selectors or compatibility ideographs that could be folded by normalization, whether to switch `c_variant_char` to a binary collation should be re-evaluated; this plan does not preemptively design for that hypothetical scenario.
...
return new class () extends Migration {
    public function up(): void {
        ...
        // Seed data (7 rows); c_notes match the "Existing data migration" table below.
        DB::table('char_variant_map')->insert([
            ['c_variant_char' => '愼', 'c_reference_char' => '慎', 'c_strict_excluded' => 0, 'c_notes' => 'Formerly in VariantCharNormalizer::$fallbackMap; 愼/慎 has no ambiguity risk, safe to replace anywhere, including person names'],
            ['c_variant_char' => '槀', 'c_reference_char' => '稿', 'c_strict_excluded' => 0, 'c_notes' => 'Formerly in VariantCharNormalizer::$fallbackMap; 槀/稿 has no ambiguity risk, safe to replace anywhere, including person names'],
            ['c_variant_char' => '峯', 'c_reference_char' => '峰', 'c_strict_excluded' => 1, 'c_notes' => 'Formerly in TITLE_VARIANT_MAP; replacement is fine in contexts like book titles, but must be excluded from BIOG_MAIN (person\'s primary name) and ALTNAME_DATA (person\'s alternate names) — 峯 is a legitimate person-name character and should not be forcibly rewritten'],
            ['c_variant_char' => '靑', 'c_reference_char' => '青', 'c_strict_excluded' => 0, 'c_notes' => 'Formerly in TITLE_VARIANT_MAP; 靑/青 has no ambiguity risk, safe to replace anywhere, including person names'],
            ['c_variant_char' => '頴', 'c_reference_char' => '穎', 'c_strict_excluded' => 0, 'c_notes' => 'Formerly in TITLE_VARIANT_MAP; 頴/穎 has no ambiguity risk, safe to replace anywhere, including person names'],
            ['c_variant_char' => '淸', 'c_reference_char' => '清', 'c_strict_excluded' => 0, 'c_notes' => 'New; 淸/清 has no ambiguity risk, safe to replace anywhere, including person names'],
            ['c_variant_char' => '厰', 'c_reference_char' => '廠', 'c_strict_excluded' => 0, 'c_notes' => 'New; 厰/廠 has no ambiguity risk, safe to replace anywhere, including person names'],
        ]);
    }
...
- **Follow-up cleanup of the `VariantCharNormalizer` class itself**: the `pinyin` table has already been supplemented with readings for all 7 of `$fallbackMap`'s original characters (菴, 攷, 嶽, 愼, 註, 于, 槀) (see `/app/codes/pinyin`), so `VariantCharNormalizer::normalize()`'s indirection layer has already lost its reason to exist — every character can now be looked up directly in the `pinyin` table for its correct reading, with no need for character substitution before the lookup. `app/Services/VariantCharNormalizer.php` can be deleted entirely (`$fallbackMap`, `normalize()`, `ensureLoaded()`, `reset()`, `getMappingCount()`), and calls to `normalize()` can be removed from its call sites (`BiogMainRepository::auto_pinyin()`, `ApiController::buildPinyinWord()`, `AdminBatchLoadBookTitlesController::buildPinyin()`/`collectUnpinyinableHan()`). **This cleanup's prerequisite has already been satisfied** and can be executed as an independent follow-up task apart from this table's schema implementation; it is out of scope for this document. **Open item (for that cleanup task)**: `BiogMainRepository::auto_pinyin()` uses the normalized name to do longest-prefix surname matching, and that matching logic depends on the `pinyin` table's `c_lastname` column semantics, not merely on "does a reading exist." Before removing this class, it also needs to be confirmed that the `c_lastname` value on the `pinyin` table rows for these 7 characters has been correctly set (particularly for cases where these characters actually occur in a surname context) — verifying that a reading exists is not sufficient on its own.
...
- **BIOG_MAIN replacement must preserve the `c_name_chn === c_surname_chn.c_mingzi_chn` invariant**: see the open item under point 2 of "Call-site integration direction" above — the replacement order must be "replace the `c_surname_chn`/`c_mingzi_chn` component fields first, then compose `c_name_chn`," not just replacing the composed `c_name_chn`, or the component and composed fields will diverge and break the current invariant.
