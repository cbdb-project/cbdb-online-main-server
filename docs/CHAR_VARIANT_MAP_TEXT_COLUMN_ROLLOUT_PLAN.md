...
- **已被閘門下架、且有 React／v2 替代品的 legacy Blade 寫入路徑**（使用者決定：這些頁面之後都會刪除）。指 `BasicInformation*Controller` 的 12 組子資源 `store()`／`update()`／`updateQuery()`／`destroy*()`（含 `BasicInformationEntriesController::update():379`／`updateQuery():648` 與 `BasicInformationSocialInstController:289`／`:481` 那幾份與 repository 並存的獨立實作）、`BasicInformationController::store()`／`update()`、`BasicInformationProposalController::normalizePayloadForTable()`（它自己直接呼叫 `replaceStrict()`、不經 repository，所以 S3 的 repository 改動不會弄壞它，且會自動繼承 S1 對 `strictMap()` 的閉包改動）、以及只被它們呼叫的 `BiogMainRepository::altnameStoreById()`／`altnameUpdateById()`。當時的狀態是：全部 flag 皆為 `new`，這些端點的非 GET 都被 `LegacyBladeFormGate` 擋成 410。✅ **2026-09-14 後續（Blade 下架環節 2／7b）**：上面列舉的 legacy `store()`／`update()`／`updateQuery()`／`destroy*()` **controller 方法**、12 組子資源的 controller 與路由、`BasicInformationProposalController` 整檔、`LegacyBladeFormGate`、15 個 `MIGRATION_FLAG_BASICINFO_*` flag，以及 `BiogMainRepository::altnameStoreById()`／`altnameUpdateById()` **都已實體刪除**——**不再是「被閘門擋住但碼還在」，而是程式不存在**，也沒有任何 flag 能把它們翻回來。
  ⚠️ **精確邊界**：① `BasicInformationController` **本體仍在**（承載 `/app` 端點，且 `DELETE /basicinformation/{id}` 仍指向它的 `destroy()`——見 Blade 下架計畫 D-5b）；② 人物主檔的舊 URI 本身**保留為 shim**：`POST /basicinformation` 與 `PUT|PATCH /basicinformation/{id}` 的路由 closure 固定 `abort(410)`、`GET` 那幾條 302 導向 `/app`。③ 那個 410 是 **closure 的行為**，不是外部可觀測的固定回應——`VerifyCsrfToken` 屬 `web` group、跑在路由之前，所以真實世界未帶 token 的 POST 會先拿到 **419**。12 個子資源都有 React 編輯器與 v2 handler。
- 既有資料的批次回溯校正（D6）。
- `OperationsController::restore()` 的**內容替換**（S1 仍在它掛 `assertWritable()` 結構驗證）。
- 眾包 create 提交的 Web UI 端：當時靠 `basicinformation.editor` flag = `new` 讓 legacy POST 回 410；v2 `BiogMainCreateHandler` 對 `mode=proposal` 回 501。✅ **2026-09-14 後續**：該 flag 已實體刪除（環節 2），`POST /basicinformation` 改為由**路由 closure 直接 `abort(410)`**（不再經 controller、不再受任何 flag 影響；未帶 CSRF token 時會先被 `VerifyCsrfToken` 擋成 419），`appCreate()` 改為 `abort(501)` 並附說明（原本會無限導向）。**永久不可達，不只是「現行設定下」。**（token API 那條仍活著且無替代品，在 S7 範圍內。）
