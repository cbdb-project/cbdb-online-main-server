...
- [x] **(§D-6)** Tests: backend `ApiV2MutateCodeTablesTest` / `CodesControllerTest` / `AdminBatchLoadBookTitlesTest`; frontend `pinyinUmlaut.test.ts` (`collectUmlautConversions` multi-column detection) + `CodesCreateInertiaTest` (`tier2_fields` prop: `['c_name']` for a config table, `[]` for a non-config table).
