...
- [x] **(§D-6)** save-time v→ü normalization — **all three backend hooks done**: (1) `AbstractCodeTableMutationHandler`/`ConfigCodeTableMutationHandler` (v2 API); (2) `CodesController` (`store`/`update` / proposal store+update; normalizes only config `tier1_fields`, leaves Tier 2, untouched for non-Phase-B tables); (3) `AdminBatchLoadBookTitlesController::updatePinyin()` (book-title `c_title`, Tier 1). **Frontend Tier-2 dialog in a later milestone**
- [x] **(§D-6)** Backend tests: `ApiV2MutateCodeTablesTest` (handler Tier 1/2 + idempotence), `CodesControllerTest` (store/update Tier-1 convert, Tier-2 not, non-config table untouched), `AdminBatchLoadBookTitlesTest` (updatePinyin v→ü). **Frontend Tier-2 dialog tests pending**
