...
移除括號**符號**與所有空白，但**保留括號裡的內容**
（`NameSearchIndexService::normalizeName()`，正則 `/[()（）\s\p{Zs}]/u`）：
```
王安石(Wang Anshi)  → 王安石WangAnshi
王安石（王介甫）    → 王安石王介甫
李白 (青蓮)         → 李白青蓮
許瑤(2)             → 許瑤2
```
⚠️ **不是「移除末尾括號註釋」**——這份文件在 2026-09-22 之前寫成「`王安石(Wang Anshi) → 王安石`」，
是錯的，而且正好會讓人誤判「FTS 裡沒有括號樣式的資料」。保留內容是刻意的：括號裡常是有效的
別名或外文名，剝掉會讓它們搜不到；去空白則是因為中文姓名不含有意義的空白，殘留空白會破壞
後綴索引與比對。
