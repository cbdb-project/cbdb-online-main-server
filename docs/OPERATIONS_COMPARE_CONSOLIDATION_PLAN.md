...
> **實施前先裁定**：[PERSON_PROPOSAL_PATHS.md](./PERSON_PROPOSAL_PATHS.md) §7 提出把提案原語
> 從「行快照」改為「意圖（命令）」。若該案立項，本案的 D2（pending 現算預覽）應直接以
> 「渲染命令」實作、與核准執行共用同一條路徑，D3 的兩列收合也隨命令模型自然消失——屆時本案
> 縮減為展示層清理；若不立項，本案按下述步驟獨立實施。
