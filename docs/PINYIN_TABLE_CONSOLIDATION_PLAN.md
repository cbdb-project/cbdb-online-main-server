...
- **同一個字若兩邊都有資料，一般轉換要用哪一筆？** 本計畫的預設假設是：**優先採用 `c_lastname=0`（一般讀音），查無 `c_lastname=0` 才退回 `c_lastname=1`（姓氏讀音）**——因為一般文字語境（名字、書名等）用常用字讀音比姓氏讀音更符合直覺。**這是本計畫的假設，不是使用者已明確拍板的規則，實作前務必請提出者（使用者）確認這個優先序方向是否正確**，如果反過來（姓氏讀音優先）或需要其他規則，屬於本計畫遺留的待決策事項，見文末「待決策事項」。
...
1. **一般轉換查詢的優先序方向**：本計畫預設「同一字 `c_lastname=0`（一般讀音）優先於 `c_lastname=1`（姓氏讀音）」，這是本計畫作者的假設，不是使用者已明確拍板的規則。需要使用者確認方向是否正確，或提供更精確的規則（例如是否有已知的具體衝突字需要特別處理）。
