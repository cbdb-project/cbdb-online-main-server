...
- **✅ Phase A complete (2026-07-06)**: the pending-human items above were fixed by Hongsu in the input system; residual `v` went from 58 to 0 (5 BIOG rows are `<待删除>` deletion-marked, skipped). §D-10 wrap-up done and merged to develop (#1126): removed `'pinyin'` from `config/codes.php` `ui_hidden` (re-exposing the surname-pinyin table) and removed the M2 scan command `cbdb:scan-pinyin-v` (kept `PinyinUmlaut` and `MigratePinyinV`). Server token file deleted; rotate reminded. **§D-12 (auto-normalize v→ü on save, manual-input paths) is implemented** (two-tier: backend silently normalizes columns that are Hanyu pinyin by definition + a frontend confirm dialog for the possibly-Western `c_alt_name`; see [PINYIN_SAVE_NORMALIZE_DESIGN.md](./PINYIN_SAVE_NORMALIZE_DESIGN.md)), closing the stop-the-bleed loop together with the M1 generation guard.

### D-12 Auto-normalize v→ü on save (manual-input paths; "stop-the-bleed 2.0") ✅ Implemented
> **Gap (found by Hongsu 2026-07-02)**: M1 stop-the-bleed only hooks the **generation paths** (pinyin auto-generated from Chinese: `auto_pinyin` / the three batch `buildPinyin` / `buildPinyinWord`). But pinyin a user **manually types** in an edit form (e.g. typing `lv` directly) goes through the **save path, not generation**, so the raw `v` is stored as-is and never becomes `ü` — and would re-pollute the data after the migration.

**Implementation design & final decisions: see [PINYIN_SAVE_NORMALIZE_DESIGN.md](./PINYIN_SAVE_NORMALIZE_DESIGN.md)** (reviewed by a review agent + codex over two rounds). Key points and **corrections to this section's earlier draft**:

- **Prerequisite**: `App\Support\PinyinUmlaut::normalize()` (rule `l/n`+`v` not followed by a/i/o/u → `lü/nü/lüe/nüe`) built by M1 on develop; this adds `PinyinUmlaut::normalizeFields()` + two allowlist constants applied uniformly.
- **⚠️ Corrects the earlier field error**: this section originally listed `c_name_rm`/`c_surname_rm`/`c_mingzi_rm`/`c_*_proper` as normalization targets — **wrong**. Those are Wade-Giles romanization and native-Latin names (may contain a **real** `v`, e.g. Silva) and must **never** be converted (consistent with §4 "columns not converted"). The v→ü allowlist **must not reuse `BracketNormalizer`'s field list** (which includes `_rm`/`_proper`).
- **Two-tier mechanism (Hongsu's decision)**:
  - **Tier 1 | backend silent** — columns that are Hanyu pinyin by definition: BIOG `c_surname`/`c_mingzi`/`c_name`. (The ALTNAME `c_alt_name_pinyin`/`2`/`3` columns listed here never existed; removed in #1284.)
