...
| **M** | **寫入等價 / 幂等（新舊同輸入 → DB 落庫列等價）** | 寫入（寫，**合成資料**） | 舊 Blade 表單 POST | 新 v2 API（edit-v2 編輯器送出） | **最能抓本 session 寫入路徑惡性 bug 的維度。** 對 13 個人物編輯器：用**同一筆合成輸入**分別經 ① 舊 Blade、② 新 v2 寫入兩個獨立合成主鍵，斷言**落庫列等價**——不只主表欄位，還含**副表（EVENTS_ADDR / POSSESSION_ADDR / POSTED_TO_ADDR）與互逆鏡像列（ASSOC/KIN 的反向關係碼）**；任一欄或鏡像碼不一致即 FAIL（正能抓「新版鏡像寫 0／未详、舊版寫正確配對碼」「副表靜默不落庫」這類分歧）。**幂等**：新版同輸入重送不得產生重複列或欄位漂移；proposal 核准路徑套用後與 direct 寫入結果一致。全程合成資料、`finally` 清理，**絕不碰真實 CBDB**。可用 phpunit（同進程內兩寫入路徑比對 DB）或 e2e（舊表單 + 新 API）實作。 |

> **I/J/K 是「資料以外」維度**，補先前盲點（見 §1.5）。I 直接驗證字體實際載入；J 強制走側邊欄而非寫死 URL，抓出「flag 未翻/側邊欄仍指舊頁/登入落地錯」這類進入路徑問題。
> **M 是「寫入等價」維度**（2026-06-24 新增，見 §0.5 #6 與本 session 的 assoc 鏡像未详 / kinship 反向 / 地址副表 bug）：把**舊版寫入行為當 ground truth** 來比新版，系統性抓出單邊 phpunit（只驗新版自身正確）測不到的「新舊寫入分歧」。涵蓋主表＋副表＋互逆鏡像＋幂等＋proposal-核准一致性。
...
| BasicInformation 子資源 新增/改/刪（UI 外殼） | ⚠️ 原述「新只有唯讀 index」**已過時**：本 session 已建 13 個完整 React edit-v2 寫入編輯器 | 外殼 HTML 差異大，不做逐字 UI 對比；但**寫入結果必須做等價對比**——見 §二 流程 **M**（新舊同輸入 → DB 落庫列等價，含副表/互逆鏡像/幂等），不再只靠單邊 API phpunit |
