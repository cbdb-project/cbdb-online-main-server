<template>
    <div class="">
        <div class="form-group">
            <div class="text-center">查询机构单位</div>
        ...
        <table class="table table-hover table-condensed">
            ...
            <tr>
                <th>c_office_id</th>
                <th>c_office_chn</th>
                <th>c_office_pinyin</th>
                <th>操作</th>
            </tr>
            </thead>
            <tbody>
            <tr v-for="item in names.data">
                <td>{{item.c_office_id}}</td>
                <td>{{item.c_office_chn}}</td>
                <td>{{item.c_office_pinyin}}</td>
                <td>
                    <div class="btn-group">
                        <a type="button" class="btn btn-sm btn-info" :href="'/officecodes/'+item.c_office_id+'/edit'">edit</a>
...
<script>
    export default {
        ...
        methods: {
            searchByName(val = 1) {
                axios.post('/api/officecode', {'q': this.q, 'page': val}).then(response => {
