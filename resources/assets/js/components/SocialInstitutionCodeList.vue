<template>
    <div class="">
        <div class="form-group">
            <div class="text-center">查询社会机构</div>
        ...
        <table class="table table-hover table-condensed">
            ...
            <thead>
            <tr>
                <th>c_inst_name_code</th>
                <th>c_inst_code</th>
                <th>c_inst_type_code</th>
                <th>操作</th>
            </tr>
            </thead>
            <tbody>
            <tr v-for="item in names.data">
                <td>{{item.c_inst_name_code}}</td>
                <td>{{item.c_inst_code}}</td>
                <td>{{item.c_inst_type_code}}</td>
                <td>
                    <div class="btn-group">
                        <a type="button" class="btn btn-sm btn-info" :href="'/socialinstitutioncodes/'+item.c_inst_name_code+'-'+item.c_inst_code+'-'+item.c_inst_type_code+'/edit'">edit</a>
...
<script>
    export default {
        ...
        methods: {
            searchByName(val = 1) {
                axios.post('/api/socialinstitutioncode', {'q': this.q, 'page': val}).then(response => {
