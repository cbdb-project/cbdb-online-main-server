...
interface BatchOfficesPageProps extends SharedProps {
    input: string;
    results: ResultRow[];
    batch_errors: string[];
...
const COLS = [
    'batch_col_line', 'batch_col_office_id', 'batch_col_name_chn', 'batch_col_name_en',
    'batch_col_pinyin', 'batch_col_dynasty_code', 'batch_col_type_id', 'batch_col_unit', 'batch_col_source_textid',
];

export default function BatchLoadOffices() {
    const props = usePage<BatchOfficesPageProps>().props;
    ...
    return (
        <DashboardLayout title={t('batch_offices_page_title')}>
            <div className="rounded-lg border border-border bg-card p-4">
                <p className="mb-3 text-sm text-muted-foreground" dangerouslySetInnerHTML={{ __html: t('batch_offices_desc') }} />
                            ...
                            <tbody>
                                {results.map((r) => (
                                    <tr key={`${r.line}-${r.office_id}`} className="border-t border-border">
                                        ...
                                        <td className="px-3 py-1.5">{r.type_id}</td>
