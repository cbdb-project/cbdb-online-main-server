...
import { InstitutionUrls } from './InstitutionForm';

type Row = Record<string, unknown>;
type Params = Record<string, FormDataConvertible>;

/**
 * 社會機構實體列表：與 app/codes/SOCIAL_INSTITUTION_CODES 裸表頁 feature parity（全欄位、
 * 排序、逐欄／布林篩選、關鍵字搜尋、朝代標籤），另加聚合特有的機構名（joined）與地址數
 * 計算欄，及走 mutation API 的編輯／刪除入口。與 Office/Index 同構。
 */
...
const DYNASTY_COLUMNS = ['c_inst_begin_dy', 'c_inst_floruit_dy', 'c_inst_end_dy'];
...
export default function SocialInstitutionIndex() {
    ...
    const cellValue = (row: Row, col: string): string => {
        let v = row[col];
        if (DYNASTY_COLUMNS.includes(col) && v !== '' && v != null) {
            const key = String(v);
    ...
    const del = async (id: number) => {
        ...
        try {
            const res = await fetch(urls.api_delete, {
                ...
                body: JSON.stringify({ resource: 'social-institution', person_id: 0, target: { pk: { c_inst_code: id } } }),
            });
            if (!res.ok) {
                ...
                <table className="w-full text-sm">
                    ...
                    <tbody>
                        {rows.length === 0 ? (
                            ...
                            rows.map((row) => {
                                const id = Number(row.c_inst_code);
                                return (
