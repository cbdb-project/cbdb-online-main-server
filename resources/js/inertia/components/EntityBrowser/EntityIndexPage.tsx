...
/**
 * 實體聚合列表頁通用組件（docs/ENTITY_AGGREGATE_ARCHITECTURE.md §6.5）。
 *
 * 與 EntityTableBrowser（後端）成對：吃其 props（thead/rows/meta/filters/sort/boolean…）
 * 渲染「與 codes 裸表頁 feature parity」的列表 UI——排序表頭＋PK 徽章、逐欄篩選列、
 * 布林模式切換與錯誤/描述、關鍵字搜尋、朝代標籤、分頁——外加走 mutation API 的
 * 編輯／刪除入口。排序／篩選 UI 字串沿用 codes 翻譯群組；實體差異由 config 注入。
 * Office 與 SocialInstitution 的 Index 頁都是此組件的薄殼。
 */
export interface EntityIndexConfig {
    /** 翻譯群組（page_title_index、intro、search_placeholder、btn_ 與 delete_ 系列等鍵所在）。 */
    i18nGroup: string;
    /** mutation API 的 resource 名（刪除請求用）。 */
    resource: string;
    /** 識別鍵欄名（列 key、刪除 pk）。 */
    pkField: string;
    /** 以「代碼 - 朝代名」顯示的欄位。 */
    dynastyColumns: string[];
}
...
export default function EntityIndexPage({ config }: { config: EntityIndexConfig }) {
    ...
    const t = useTranslation(config.i18nGroup);
    ...
    const cellValue = (row: Row, col: string): string => {
        let v = row[col];
        if (config.dynastyColumns.includes(col) && v !== '' && v != null) {
            const key = String(v);
    ...
    const del = async (id: number) => {
        ...
        try {
            const res = await fetch(urls.api_delete, {
                ...
                body: JSON.stringify({ resource: config.resource, person_id: 0, target: { pk: { [config.pkField]: id } } }),
            });
            if (!res.ok) {
                ...
                <table className="w-full text-sm">
                    ...
                    <tbody>
                        {rows.length === 0 ? (
                            ...
                            rows.map((row) => {
                                const id = Number(row[config.pkField]);
                                return (
