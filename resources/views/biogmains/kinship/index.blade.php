...
@section('content')
    ...
    <div class="card card-default">
        <div class="card-header">
            <h3 class="card-title">{{ __('biogmains.kinship_list') }}</h3>
        </div>

        <div class="card-body">
            @auth
                @if(Auth::user()->isActive())
                    <a href="{{ route('basicinformation.kinship.create', ['basicinformation' => $basicinformation->c_personid]) }}" class="btn btn-secondary float-right">{{ __('common.add') }}</a>
                @endif
            @endauth
            <div class="table-responsive">
                <table class="table table-hover table-sm">
                <caption>{{ __('biogmains.record_count', ['count' => $basicinformation->kinship_count]) }}</caption>
                <thead>
                <tr>
                    <th>{{ __('person.seq_no') }}</th>
                    <th>{{ __('biogmains.kinship_type') }}</th>
                    <th>{{ __('biogmains.relative_name') }}</th>
                    @auth
                        @if(Auth::user()->isActive())
                        ...
                        @auth
                            @if(Auth::user()->isActive())
                                <td>
                                    ...
                                    @endphp
                                    <div class="btn-group">
                                        <a type="button" class="btn btn-sm btn-info" href="{{ CompositePrimaryKey::buildUrl('basicinformation.kinship.edit.query', ['id' => $basicinformation->c_personid], $kinPk) }}">{{ __('common.edit') }}</a>
                                        <a href=""
                                           onclick="
