...
class ApiV2CreateTextTest extends TestCase {
    ...
    #[Test]
    public function testDirectTextCreateWritesOperationRecord(): void {
        $user = $this->makeUser(email: 'create-text-op@example.com');
        $this->actingAs($user);
    ...
    #[Test]
    public function testDirectTextCreateWritesAuditLog(): void {
        $user = $this->makeUser(email: 'create-text-audit@example.com');
        $this->actingAs($user);
    ...
    #[Test]
    public function testProposalTextCreateSucceeds(): void {
        $user = $this->makeUser(User::STATUS_ACTIVE, User::ROLE_CROWDSOURCING, 'create-text-proposal@example.com');
        $this->actingAs($user);
    ...
    #[Test]
    public function testProposalTextCreateWritesProposalMeta(): void {
        $user = $this->makeUser(User::STATUS_ACTIVE, User::ROLE_CROWDSOURCING, 'create-text-prop-meta@example.com');
        $this->actingAs($user);
        ...
        $this->assertSame('提案新增著述', $resourceData['__proposal_meta']['comment']);
    ...
    public function testCreateDuplicateTargetPkReturns409(): void {
        $user = $this->makeUser(email: 'create-text-dup@example.com');
        $this->actingAs($user);
        ...
        $response->assertStatus(409)->assertJson(['ok' => false]);
    }

    #[Test]
    public function testCreateDuplicatePendingProposalReturns409(): void {
        $user = $this->makeUser(User::STATUS_ACTIVE, User::ROLE_CROWDSOURCING, 'create-text-dup-prop@example.com');
        $this->actingAs($user);
        ...
        $this->assertSame(1, DB::table('operations')->where('resource', 'BIOG_TEXT_DATA')->count());
    }

    #[Test]
    public function testCreateWithPersonIdMismatchReturns422(): void {
        $user = $this->makeUser(email: 'create-text-mismatch@example.com');
        $this->actingAs($user);

        $response = $this->postJson('/api/v2/create', $this->createPayload(['person_id' => 9999]));
    ...
    public function testCreateRejectsDisallowedField(): void {
        $user = $this->makeUser(email: 'create-text-disallowed@example.com');
        $this->actingAs($user);
    ...
    public function testCreateRejectsInactiveUser(): void {
        $user = $this->makeUser(User::STATUS_INACTIVE, User::ROLE_REGULAR, 'create-text-inactive@example.com');
        $this->actingAs($user);
    ...
    public function testDirectCreateRejectsCrowdsourcingUser(): void {
        $user = $this->makeUser(User::STATUS_ACTIVE, User::ROLE_CROWDSOURCING, 'create-text-crowd@example.com');
        $this->actingAs($user);
