...
class ApiV2DeleteEventTest extends TestCase {
    ...
    #[Test]
    public function testDirectEventDeleteSucceeds(): void {
        $user = $this->makeUser(email: 'delete-event-direct@example.com');
        $this->actingAs($user);
        $this->seedEvent();
    ...
    #[Test]
    public function testDirectEventDeleteWritesOperationAndAudit(): void {
        $user = $this->makeUser(email: 'delete-event-op@example.com');
        $this->actingAs($user);
        $this->seedEvent();
    ...
    #[Test]
    public function testProposalEventDeleteWritesPendingProposal(): void {
        $user = $this->makeUser(User::STATUS_ACTIVE, User::ROLE_CROWDSOURCING, 'delete-event-proposal@example.com');
        $this->actingAs($user);
        $this->seedEvent();
        ...
        $this->assertDatabaseHas('EVENTS_DATA', ['c_personid' => 1000, 'c_sequence' => 1, 'c_event_code' => 40]);

        $this->assertSame(0, DB::table('audit_log')->where('table_name', 'EVENTS_DATA')->where('operation', 'DELETE')->count());
    }

    #[Test]
    public function testDeleteTargetMissingReturns404(): void {
        $user = $this->makeUser(email: 'delete-event-404@example.com');
        $this->actingAs($user);
    ...
    public function testDeleteWithPersonIdMismatchReturns422(): void {
        $user = $this->makeUser(email: 'delete-event-mismatch@example.com');
        $this->actingAs($user);
        $this->seedEvent();
    ...
    public function testDeleteRejectsInactiveUser(): void {
        $user = $this->makeUser(User::STATUS_INACTIVE, User::ROLE_REGULAR, 'delete-event-inactive@example.com');
        $this->actingAs($user);
        $this->seedEvent();
    ...
    public function testDirectDeleteRejectsCrowdsourcingUser(): void {
        $user = $this->makeUser(User::STATUS_ACTIVE, User::ROLE_CROWDSOURCING, 'delete-event-crowd@example.com');
        $this->actingAs($user);
        $this->seedEvent();
