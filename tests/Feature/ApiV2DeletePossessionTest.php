...
class ApiV2DeletePossessionTest extends TestCase {
    ...
    protected function seedPossession(): void {
        ...
        DB::table('POSSESSION_ADDR')->insert([
            ['c_personid' => 1000, 'c_possession_record_id' => 5, 'c_addr_id' => 130],
            ['c_personid' => 1000, 'c_possession_record_id' => 5, 'c_addr_id' => 200],
        ]);
    }
    ...
    protected function deletePayload(array $overrides = []): array {
        return array_replace_recursive([
            ...
            'person_id' => 1000,
            'mode' => 'direct',
            'target' => ['pk' => ['c_possession_record_id' => 5]],
        ], $overrides);
    }

    #[Test]
    public function testDirectPossessionDeleteSucceedsAndRemovesSideTable(): void {
        $user = $this->makeUser(email: 'delete-poss-direct@example.com');
        $this->actingAs($user);
        $this->seedPossession();
        ...
        $this->assertDatabaseMissing('POSSESSION_DATA', ['c_possession_record_id' => 5]);
        $this->assertSame(0, DB::table('POSSESSION_ADDR')->where('c_possession_record_id', 5)->count());
    }

    #[Test]
    public function testDirectPossessionDeleteWritesOperationAndAudit(): void {
        $user = $this->makeUser(email: 'delete-poss-op@example.com');
        $this->actingAs($user);
        $this->seedPossession();
    ...
    #[Test]
    public function testProposalPossessionDeleteWritesPendingProposal(): void {
        $user = $this->makeUser(User::STATUS_ACTIVE, User::ROLE_CROWDSOURCING, 'delete-poss-proposal@example.com');
        $this->actingAs($user);
        $this->seedPossession();
        ...
        $this->assertDatabaseHas('POSSESSION_DATA', ['c_possession_record_id' => 5]);
        $this->assertGreaterThan(0, DB::table('POSSESSION_ADDR')->where('c_possession_record_id', 5)->count());

        $this->assertSame(0, DB::table('audit_log')->where('table_name', 'POSSESSION_DATA')->where('operation', 'DELETE')->count());
    }

    #[Test]
    public function testDeleteTargetMissingReturns404(): void {
        $user = $this->makeUser(email: 'delete-poss-404@example.com');
        $this->actingAs($user);
    ...
    public function testDeleteWithPersonIdMismatchReturns422(): void {
        $user = $this->makeUser(email: 'delete-poss-mismatch@example.com');
        $this->actingAs($user);
        $this->seedPossession();
    ...
    public function testDeleteRejectsInactiveUser(): void {
        $user = $this->makeUser(User::STATUS_INACTIVE, User::ROLE_REGULAR, 'delete-poss-inactive@example.com');
        $this->actingAs($user);
        $this->seedPossession();
    ...
    public function testDirectDeleteRejectsCrowdsourcingUser(): void {
        $user = $this->makeUser(User::STATUS_ACTIVE, User::ROLE_CROWDSOURCING, 'delete-poss-crowd@example.com');
        $this->actingAs($user);
        $this->seedPossession();
