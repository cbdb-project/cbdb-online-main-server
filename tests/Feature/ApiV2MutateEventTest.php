...
class ApiV2MutateEventTest extends TestCase {
    ...
    public function testUpdatePreservesCreationAuditFieldsAndStampsModification(): void {
        $this->actingAs($this->makeUser(email: 'event-audit@example.com'));
        $this->seedEvent([
        ...
        $this->postJson('/api/v2/mutate', $this->eventPayload())->assertOk();
    ...
    public function testClientSuppliedCreationAuditFieldsCannotOverwriteThem(): void {
        $this->actingAs($this->makeUser(email: 'event-audit-injected@example.com'));
        $this->seedEvent([
            'c_created_by' => 'original-author',
            'c_created_date' => '2020-01-01',
        ]);
