...
class ApiV2MutatePossessionTest extends TestCase {
    ...
    /**
     * personid 0 不能**修改**財產記錄——對齊 PossessionCreateHandler:89 的 create 側守衛。
     *
     * legacy BasicInformationPossessionController 兩側都沒擋，所以這不是遷移漏搬，
     * 而是 v2 自己補到一半。三條入口都要擋：一般 update、僅改地址 direct、僅改地址 proposal。
     */
    #[Test]
    public function testUnknownPersonCannotUpdatePossession(): void {
        $this->actingAs($this->makeUser(email: 'poss-unknown@example.com'));
        $this->seedPossession(['c_personid' => 0, 'c_notes' => '髒列']);
        ...
        // 真的沒寫進去（不是只回錯誤碼卻照樣落庫）。
        $this->assertSame('髒列', DB::table('POSSESSION_DATA')->where('c_possession_record_id', 500)->value('c_notes'));
    }

    /** 僅改地址的兩條快捷路徑不經 parent::handle()，守衛必須各自掛一次。 */
    #[Test]
    public function testUnknownPersonCannotUpdatePossessionAddressesOnly(): void {
        $this->seedPossession(['c_personid' => 0]);
        $this->seedPossessionAddr(130);

        foreach (['direct', 'proposal'] as $mode) {
            $this->actingAs($this->makeUser(email: "poss-unknown-addr-{$mode}@example.com"));
            $this->postJson('/api/v2/mutate', [
                'resource' => 'possessions',
                'person_id' => 0,
                'mode' => $mode,
                'operation' => 'update',
                'target' => ['pk' => ['c_possession_record_id' => 500]],
                'changes' => ['c_addr_id' => [140]],
            ])
                ->assertStatus(422)
                ->assertJsonPath('errors.person_id.0', 'invalid');
        }
        ...
        // 地址副表完全沒動。
        $this->assertDatabaseHas('POSSESSION_ADDR', ['c_possession_record_id' => 500, 'c_addr_id' => 130]);
        $this->assertDatabaseMissing('POSSESSION_ADDR', ['c_possession_record_id' => 500, 'c_addr_id' => 140]);
    }
     ...
     */
    #[Test]
    public function testUnknownPersonPossessionGuardRunsAfterAuthorization(): void {
        $this->actingAs($this->makeUser(User::STATUS_ACTIVE, User::ROLE_CROWDSOURCING, 'poss-unknown-authz@example.com'));
        $this->seedPossession(['c_personid' => 0]);
